...
class ParseDemoRequest(BaseModel):
    ...
    url: Optional[str] = Field(None, description="URL для парсинга (если не передан, берётся из config)")
...
class CompetitorAnalysis(BaseModel):
    ...
    design_score: int = Field(0, ge=0, le=10, description="Оценка дизайна и визуальной подачи (0-10)")
    animation_potential: int = Field(0, ge=0, le=10, description="Потенциал для анимации/моушн-улучшений (0-10)")
...
class ImageAnalysis(BaseModel):
    ...
    recommendations: List[str] = Field(default_factory=list, description="Рекомендации")
    design_score: int = Field(0, ge=0, le=10, description="Оценка дизайн-решения (0-10)")
    animation_potential: int = Field(0, ge=0, le=10, description="Потенциал улучшения через анимацию (0-10)")
