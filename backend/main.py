...
@app.post("/analyzetext", response_model=TextAnalysisResponse)
...
@app.post("/analyzeimage", response_model=ImageAnalysisResponse)
...
@app.post("/parsedemo", response_model=ParseDemoResponse)
async def parse_demo(request: ParseDemoRequest):
    ...
    if not target_url:
        if not settings.competitor_urls:
            return ParseDemoResponse(
                success=False,
                error="В config.py не задан competitor_urls и URL не передан в запросе"
            )
        target_url = settings.competitor_urls[0]
        logger.info(f"  URL не передан, используем из config.py: {target_url}")
    else:
    ...
    try:
        ...
        title, h1, first_paragraph, screenshot_bytes, error = await parser_service.parse_url(target_url)
        ...
        history_service.add_entry(
            request_type="parse",
            request_summary=f"URL: {target_url}",
